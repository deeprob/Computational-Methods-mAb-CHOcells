...
myOutputs = pd.concat([Glycan_Amb,Glycan_CD],axis=0).astype(float).reset_index(drop=True)
myOutputsGrp = pd.concat([Glycan_Amb_grp,Glycan_CD_grp],axis=0).astype(float).reset_index(drop=True)
myOutputsFuc = pd.concat([Glycan_Amb_fuc,Glycan_CD_fuc],axis=0).astype(float).reset_index(drop=True)
